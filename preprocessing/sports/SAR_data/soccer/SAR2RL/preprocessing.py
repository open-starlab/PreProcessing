...
# Keep this module lightweight: avoid importing the full SAR preprocessing stack (and its
# optional dependencies) just to read precomputed `events.jsonl` outputs.
...
_AGENT_ORDERING_RULE = "ascending_player_id"
...
def _action_to_id(action: Any, action_to_id: Mapping[str, int], default_id: int) -> int:
    ...
    if not a:
        return default_id
    return int(action_to_id.get(a.lower(), default_id))
...
def _infer_onball_mask(
    ...
    *,
    ...
    for i, pid in enumerate(agent_ids):
        action_name = str(attackers_by_id.get(pid, {}).get("action", "") or "").lower()
        if action_name in _ONBALL_ACTIONS:
            explicit[i] = True
    ...
    if float(dists[j]) <= float(onball_distance_threshold):
        onball = np.zeros((n_agents,), dtype=np.bool_)
        onball[j] = True
        return onball
    return np.zeros((n_agents,), dtype=np.bool_)
...
def _build_state_from_observation(observation: np.ndarray) -> np.ndarray:
    max_seq_len, n_agents, obs_dim = observation.shape
    return observation.reshape(max_seq_len, n_agents * obs_dim)


def _shift_valid_timesteps(values: np.ndarray) -> np.ndarray:
...
def _convert_sequence(
    ...
    *,
    ...
    split_key = game_id if config.split_by == "game_id" else seq_uid
    ...
    if len(events) > config.max_seq_len:
        events = events[-config.max_seq_len :] if config.truncate == "tail" else events[: config.max_seq_len]
    ...
    try:
        ...
        logger.debug("Skipping sequence %s: %s", seq_uid, e)
        return None
    ...
    for t in range(t_steps):
        ...
        if not isinstance(ev, Mapping):
            return None
        state_t = ev.get("state")
        if not isinstance(state_t, Mapping):
            return None
        raw = _extract_raw_state(state_t)

        ball = _get_ball(raw)
        ball_xy = _get_pos_xy(ball)
        ...
        for player in attack_players:
            ...
            try:
                ...
                continue
            attackers_by_id[pid_i] = player

        if config.strict_agents and any(pid not in attackers_by_id for pid in agent_ids):
            logger.debug("Skipping sequence %s: missing agent at t=%s", seq_uid, t)
            return None
        ...
        for j, pid in enumerate(agent_ids):
            player = attackers_by_id.get(pid)
            if player is None:
                continue
            actions[t, j] = _action_to_id(player.get("action"), config.action_to_id, config.unknown_action_id)
            obs[t, j, :] = _build_obs_raw_pvs_basic(player, ball, onball=bool(onball_t[j]))
    ...
    next_observation = _shift_valid_timesteps(obs)
    next_state = _shift_valid_timesteps(state)
...
def build_rl_datasets_from_sar_events(
    ...
    *,
    ...
    meta = {
        ...
        "agent_ordering": {
            "deterministic": True,
            "rule": _AGENT_ORDERING_RULE,
            "n_agents": cfg.n_agents,
        },
        "compatibility": {
            "shared_dataset_for": ["dqn", "qmix"],
            "dqn_loader_note": "Flatten attacker dimension N into the sample dimension at load time.",
            "qmix_state_note": "Use explicit state and next_state tensors for centralized training.",
            "shared_reward_note": "DQN and QMIX use the same team reward, splits, seed, PAD id, and action mapping.",
            "unavailable_action_note": "Apply the same unavailable-action rules for both models using onball_mask.",
        },
        "observation_schema": {
            "name": "raw_pvs_basic",
            "dim": obs_dim,
        ...
        "state_schema": {
            "name": "ordered_attacker_observation_concat",
            "dim": cfg.n_agents * obs_dim,
            "base_observation": "raw_pvs_basic",
            "agent_ordering_rule": _AGENT_ORDERING_RULE,
        },
    }
    ...
    for split_name, samples in splits.items():
        ...
        np.savez_compressed(
            ...
            observation=np.stack([s["observation"] for s in samples], axis=0),
            next_observation=np.stack([s["next_observation"] for s in samples], axis=0),
            state=np.stack([s["state"] for s in samples], axis=0),
            next_state=np.stack([s["next_state"] for s in samples], axis=0),
            action=np.stack([s["action"] for s in samples], axis=0),
            reward=np.stack([s["reward"] for s in samples], axis=0),
            done=np.stack([s["done"] for s in samples], axis=0),
            ...
            agent_ids=np.stack([s["agent_ids"] for s in samples], axis=0),
        )
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build DQN/QMIX datasets from SAR `events.jsonl` outputs.")
    parser.add_argument("--sar-preprocessed-dir", type=str, required=True, help="Directory containing `**/events.jsonl`.")
    parser.add_argument("--output-dir", type=str, required=True, help="Output directory for .npz + meta.json.")
    ...
    parser.add_argument("--pad-action-id", type=int, default=DEFAULT_PAD_ACTION_ID)
    parser.add_argument("--unknown-action-id", type=int, default=0)
    parser.add_argument(
        "--action-map-json",
        type=str,
        default=None,
    ...
    parser.add_argument("--split-by", type=str, choices=["game_id", "sequence_id"], default="game_id")
    parser.add_argument("--train-ratio", type=float, default=0.8)
    parser.add_argument("--val-ratio", type=float, default=0.1)
    parser.add_argument("--test-ratio", type=float, default=0.1)
    parser.add_argument("--onball-threshold", type=float, default=2.0)
    parser.add_argument("--strict-agents", dest="strict_agents", action="store_true", help="Require exactly N attackers.")
    parser.add_argument("--no-strict-agents", dest="strict_agents", action="store_false", help="Allow missing attackers.")
    parser.set_defaults(strict_agents=True)
    parser.add_argument("--truncate", type=str, choices=["tail", "head"], default="tail")
    ...
    if args.action_map_json:
        ...
        for key, value in raw.items():
            if key is None:
                continue
            try:
                action_map[str(key).strip().lower()] = int(value)
            except (TypeError, ValueError):
                raise ValueError(f"Invalid action map entry: {key} -> {value}") from None
...
__all__ = [
    ...
    "DEFAULT_PAD_ACTION_ID",
    "DEFAULT_VOCAB_SIZE",
    "SoccerRLDatasetConfig",
