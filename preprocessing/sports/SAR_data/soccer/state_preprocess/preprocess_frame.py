...
from preprocessing.sports.SAR_data.soccer.state_preprocess.state_EDMF import calc_absolute_state, calc_offball, calc_onball
...
def frames2events_PVF(
    ...
    for idx in range(attack_start_history_num_list_len):
        ...
        states = current_frames.apply(frame2state_PVF, axis=1, args=(team_name_attack, origin_pos, absolute_coordinates, league))
        rewards = reward_model.calculate_reward(league, current_frames, next_frames)
        events = [Event_PVF(state=state, reward=reward) for state, reward in zip(states, rewards)]
...
def frame2state_EDMF(
    frame: pd.Series, team_name_attack: str, origin_pos: str = "center", absolute_coordinates: bool = False, league: str = "laliga", prev_team: str = None
) -> State_EDMF:
    state = frame['state']
    if league == "laliga":
    ...
    absolute_state = calc_absolute_state(players_with_action, ball, attack_players, defense_players, formation if league == "laliga" else None)
    raw_state = RawState(ball=ball, players=players_with_action, attack_players=attack_players, defense_players=defense_players)
    state = State_EDMF(relative_state=relative_state, absolute_state=absolute_state, raw_state=raw_state)
    # print(f"state: {state}")
...
def process_frame(row, team_name_attack, origin_pos, absolute_coordinates, league):
    return frame2state_PVF(row, team_name_attack, origin_pos, absolute_coordinates, league)
...
def frames2events_EDMF(
    ...
    for idx in tqdm(range(attack_start_history_num_list_len)):
        ...
        try:
            # states = parallel_frame2state(current_frames, team_name_attack, origin_pos, absolute_coordinates, league)
            # states = current_frames.apply(frame2state, axis=1, args=(team_name_attack, origin_pos, absolute_coordinates, data_type))
            previous_team = None
            states = []

            for idx, row in current_frames.iterrows():
                state, onball_team = frame2state_EDMF(
                    frame=row,
                    team_name_attack=team_name_attack,
                    origin_pos=origin_pos,
                    absolute_coordinates=absolute_coordinates,
            ...
            continue
        rewards = reward_model.calculate_reward(league, current_frames, next_frames)
        events = [Event_EDMF(state=state, reward=reward) for state, reward in zip(states, rewards)]
