...
def load_statsbomb_skillcorner(statsbomb_event_dir: str, skillcorner_tracking_dir: str, skillcorner_match_dir: str, statsbomb_match_id: str, skillcorner_match_id: str) -> pd.DataFrame:
    ...
    for _, event in events.iterrows():
        ...
        if tracking_data:
            ...
            # # 判断 tracking_data 是否包含 ball
            # contains_ball = any(obj['trackable_object'] == ball_id for obj in tracking_data)

            # if contains_ball:
            #     print("tracking_data 包含 ball")
            # else:
            #     print("tracking_data 不包含 ball")
        ...
        df_list.append([match_id, period, time, minute, second, seconds, event_type, event_type_2, team, home_team, player, start_x, start_y, end_x, end_y, *home_tracking, *away_tracking, *home_side, ball_x, ball_y])
    ...
    # Kick Off
    kick_off_events = events[events['pass_type'] == 'Kick Off']
    kick_off_player1 = kick_off_events[kick_off_events['period'] == 1]
    kick_off_player2 = kick_off_events[kick_off_events['period'] == 2]

        # find Kick Off player's team and the player nearest the cball
    def find_kickoff_player_coordinates(kick_off_players, match, tracking):
        coordinates = []

        for _, event_row in kick_off_players.iterrows():
            player_name = event_row['player']
            team_name = event_row['team']
            period = event_row['period']
            timestamp = event_row['timestamp']

            # Step 1: 在 match 数据中查找对应 player_name 和 team_name 的 trackable_object
            trackable_object = None
            for team,player in match["home_team"], match['players']:
                full_name = f"{player['first_name']} {player['last_name']}"
                if full_name == player_name:

                    tacking_data_firsthalf = tracking_data[tracking_data["type"]=="Kick_off",tracking_data["period"]==1][0]
                    x = tacking_data_firsthalf["x"]
                    y = tacking_data_firsthalf["y"]
                    delta_x = player_x - ball_x
                    delta_y = player_y - ball_y   
                    return np.sqrt(delta_x**2 + delta_y**2)
                        ...
                        if team_name == home_team_name:
                            for player in tracking_data[tracking_data["possession"]["team"]]== match["home_team"]:
                                player_x = player["x"]
                                player_y = player["y"]
                                delta_x = player_x - ball_x
                                delta_y = player_y - ball_y                              
                        ...
                        if team_name == away_team_name:   
                            for player in tracking_data[tracking_data["possession"]["team"]]== match["away_team"]:
                                player_x = player["x"]
                                player_y = player["y"]
                                delta_x = player_x - ball_x
                                delta_y = player_y - ball_y                              
            ...
            if trackable_object is None:
                continue  # 如果没有找到对应的 trackable_object，跳过此事件
    ...
    columns = ["match_id", "period", "time", "minute", "second", 'seconds', "event_type", "event_type_2", "team", "home_team", "player", "start_x", "start_y","end_x","end_y"] + home_tracking_columns + away_tracking_columns + ["home_side", "ball_x", "ball_y"]
