...
def et_plot(df, row, save_path):
    ...
    ball_x = row_df["ball_x"]
    ball_y = row_df["ball_y"]   
    ax.plot(ball_x,ball_y,'.', color='green')

    
    #plot the event location
    if row_df["start_x"] is not None:
        start_x = row_df["start_x"]
        start_y = row_df["start_y"]
        ax.plot(start_x,start_y,'.', color='purple', markersize=5)

    if row_df["end_y"] is not None:
        end_x = row_df["end_x"]
        end_y = row_df["end_y"]
        ax.plot(end_x,end_y,'.', color='black', markersize=5)

    
    # Set the figure title
    ax.set_title(f'event second: {row_df["seconds_event"]}, tracking seconds: {row_df["adjusted_seconds_tracking"]}\n red: home team, blue: away team, green: ball')
