...
def process_tracking_data(game_id, data_path, test=False):
        ...
        for idx, frame in enumerate(tqdm(frames, desc=f"Processing {team_type} team")):
            ...
            for _, player_row in frame_team_data.iterrows():
                ...
                if player_num <= 7:  # Ultimate typically has 7 players per team
                    tracking_df.loc[idx, f'{team_type}_{player_num}_x'] = player_row['x']
                    tracking_df.loc[idx, f'{team_type}_{player_num}_y'] = player_row['y']
                    tracking_df.loc[idx, f'{team_type}_{player_num}_vx'] = player_row['vx']
                    tracking_df.loc[idx, f'{team_type}_{player_num}_vy'] = player_row['vy']
                    tracking_df.loc[idx, f'{team_type}_{player_num}_ax'] = player_row['ax']
                    tracking_df.loc[idx, f'{team_type}_{player_num}_ay'] = player_row['ay']
    ...
    if game_id >= len(csv_files):
        raise ValueError(f"Game ID {game_id} out of range. Available files: {len(csv_files)}")
    ...
    required_columns = ['frame', 'id', 'class', 'x', 'y', 'vx', 'vy', 'ax', 'ay', 'closest', 'holder']
    ...
    tracking_offense = fill_tracking_data(tracking_offense, raw_data, 'offense', 'offense')
    tracking_defense = fill_tracking_data(tracking_defense, raw_data, 'defense', 'defense')
...
def analyze_possession_patterns(raw_data):
    ...
    if not possession_df.empty:
        possession_stats = possession_df.groupby('holder_team').agg({
            'frame': 'count',
            'holder_speed': 'mean',
            'holder_x': 'mean',
            'holder_y': 'mean'
...
def calculate_team_metrics(tracking_data, team_type):
    ...
    player_x_cols = [col for col in tracking_data.columns if f'{team_type}_' in col and '_x' in col]
    player_y_cols = [col for col in tracking_data.columns if f'{team_type}_' in col and '_y' in col]
...
if __name__ == "__main__":
    ...
    try:
        # Call the function
        tracking_offense, tracking_defense, team_info_df = process_tracking_data(
